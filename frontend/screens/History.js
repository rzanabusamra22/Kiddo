...
class History extends Component {
    ...
    go(x){
       ...
       if(x.kind==="Video"){this.props.sendvideo(x.link);this.props.navigation.navigate(x.kind)}
       else if(x.kind==="Game"){this.props.sendgame(x.link);this.props.navigation.navigate(x.kind)}
        else if(x.kind==="Colorings"){this.props.sendcoloring(x.link);this.props.navigation.navigate(x.kind)}
        else if(x.kind==="Draw"){this.props.senddraw(x.link);this.props.navigation.navigate(x.kind)}
    }
    render() {
        return(
...
const mapDispatchToProps = (dispatch) => {
    return {
      ...
      sendcoloring: (z) => { dispatch(sendvideo(z)) },
      senddraw: (z) => { dispatch(sendvideo(z)) },
    }
  }
